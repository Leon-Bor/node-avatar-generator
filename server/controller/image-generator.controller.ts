...
class ImageGeneratorController {
  ...
  private imagePath: string = path.join(
    __dirname,
    "../public/images/cache/" + version
  );
  private imagePath256: string = path.join(
    __dirname,
    "../public/images/cache/" + version + "/256"
  );
  ...
  public async genrateImageFromMD5(md5: string): Promise<boolean> {
    ...
    if (fs.existsSync(`${this.imagePath}/${md5}.${imageType}`)) {
        ...
        if (img) {
          ...
          img = bavatarController.getImage(
            ...
            bavatarController.dirs[i].defaultImage
              ? bavatarController.dirs[i].defaultImage.split("_")[0]
              : null
          );
      ...
      let buf = await resizeImg(fs.readFileSync(renderedImagePath), {
        width: 512,
        height: 512,
      });
